        ...
        <header>
            <nav class="bg-blue-800 text-white text-sm flex items-center justify-between xl:justify-center xl:px-32 relative">
                ...
                <div class="hidden xl:flex xl:items-center">
                    <Link href="/" :class="{ 'bg-blue-900 text-white underline': page.url === '/' }" class="px-4 py-2 md:px-4 md:py-3 hover:bg-blue-900 hover:underline">Home</Link>
                    <Link href="/news" :class="{ 'bg-blue-900 text-white underline': page.url === '/news' }" class="px-4 py-2 md:px-4 md:py-3 hover:bg-blue-900 hover:underline">News</Link>

                    <div v-for="(menu, index) in menus" :key="index" class="relative" @mouseenter="setDropdown(index, true)" @mouseleave="setDropdown(index, false)">
                        <button @click="toggleDropdown(index)" class="w-full whitespace-nowrap md:w-auto px-4 py-2 md:px-4 md:py-3 hover:bg-blue-900 hover:underline flex items-center gap-1" :class="{ 'bg-blue-900 underline': activeDropdown === index || isDropdownActive(menu) }">
                            {{ menu.name }}
                            <span class="text-xs transition-transform duration-200" :class="{ 'rotate-180': activeDropdown === index }">▼</span>
                        </button>
                        <ul v-show="activeDropdown === index" class="md:absolute left-0 top-full w-full md:w-48 bg-blue-900 text-white shadow-lg py-1.5 md:py-2 z-10">
                            <li v-for="(item, i) in menu.dropdown" :key="i" class="px-4 py-1.5 md:px-4 md:py-2 hover:underline border-t border-gray-600 first:border-none">
                                <template v-if="item.link.startsWith('http')">
                                    <a :href="item.link" class="block" target="_blank" rel="noopener noreferrer">{{ item.name }}</a>
                                </template>
                                <template v-else>
                                    <Link :href="item.link" :class="{ 'bg-blue-900 text-white underline font-bold': page.url === item.link }" class="block">{{ item.name }}</Link>
                                </template>
                            </li>
                        </ul>
                    </div>
                ...
                <transition name="mobile-menu">
                    <div v-if="isMenuOpen" class="xl:hidden absolute top-full left-0 w-full bg-blue-800 flex flex-col shadow-md">
                        <Link href="/" :class="{ 'bg-blue-900 text-white underline': page.url === '/' }" class="px-4 py-2 hover:bg-blue-900 hover:underline" @click="closeMenu">Home</Link>
                        <Link href="/news" :class="{ 'bg-blue-900 text-white underline': page.url === '/news' }" class="px-4 py-2 hover:bg-blue-900 hover:underline" @click="closeMenu">News</Link>

                        <div v-for="(menu, index) in menus" :key="index" class="relative">
                            <button @click="toggleDropdown(index)" class="w-full px-4 py-2 hover:bg-blue-900 hover:underline flex items-center gap-1" :class="{ 'bg-blue-900 underline': activeDropdown === index || isDropdownActive(menu) }">
                                {{ menu.name }}
                                <span class="text-xs transition-transform duration-200" :class="{ 'rotate-180': activeDropdown === index }">▼</span>
                            </button>
                            <transition name="dropdown-mobile">
                                <ul v-if="activeDropdown === index" class="bg-blue-900 text-white shadow-lg py-1.5">
                                    <li v-for="(item, i) in menu.dropdown" :key="i" class="px-4 py-1.5 hover:underline border-t border-gray-600 first:border-none">
                                        <template v-if="item.link.startsWith('http')">
                                            <a :href="item.link" class="block" target="_blank" rel="noopener noreferrer" @click="closeMenu">{{ item.name }}</a>
                                        </template>
                                        <template v-else>
                                            <Link :href="item.link" :class="{ 'bg-blue-900 text-white underline font-bold': page.url === item.link }" class="block" @click="closeMenu">{{ item.name }}</Link>
                                        </template>
                                    </li>
                                </ul>
                            </transition>
...
@media (max-width: 1280px) {
    .dropdown-mobile-enter-active, .dropdown-mobile-leave-active {
        transition: max-height 0.3s ease-in-out, opacity 0.3s ease-in-out;
        overflow: hidden;
    }
    .dropdown-mobile-enter-from, .dropdown-mobile-leave-to {
        max-height: 0;
        opacity: 0;
    }
    .dropdown-mobile-enter-to, .dropdown-mobile-leave-from {
        max-height: 300px;
        opacity: 1;
    }
}
